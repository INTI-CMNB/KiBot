...
class ReportOptions(VariantOptions):
    def __init__(self):
        with document:
            ...
            self.collect_images = 'auto'
            """ [auto,yes,no] When enabled we look for outputs that can generate useful images.
    ...
    def collect_data(self, board):
        ...
        for p in pad_properties:
            if p.fab_property == GS.PFP_TESTPOINT:
    ...
    def do_collect_images(self, fname):
        ...
        split_pdfs = []
        if self.collect_images == 'no' or (self.collect_images == 'auto' and fname.lower().endswith('csv')):
            # If we are generating a CSV we don't need images
            return split_pdfs
    ...
    def run(self, fname):
        ...
        split_pdfs = self.do_collect_images(fname)
