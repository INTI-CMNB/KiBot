...
from .misc import (NO_YAML_MODULE, EXIT_BAD_ARGS, EXAMPLE_CFG, WONT_OVERWRITE, W_NOOUTPUTS, W_UNKOUT, W_NOFILTERS,
                   ...
                   DEFAULT_ROTATIONS, DEFAULT_OFFSETS, W_EXTRADOCS, RE_LEN, W_DEFNOSTR, update_dict)
from .gs import GS
...
VALID_IMPORT = {'file', 'is_external', 'outputs', 'preflights', 'filters', 'variants', 'global', 'globals', 'groups',
                'definitions'}
VALID_KIBOT_SEC = {'version', 'imported_global_has_less_priority', 'merge_pre'}
RST_WARNING = ".. Automatically generated by KiBot, please don't edit this file\n"
SEE_REF = re.compile(r'([sS]ee) `([^`]+)`')
SEE_OUT = re.compile(r'([sS]ee) the `([^`]+)` output')
rst_mode = False
...
class CfgYamlReader(object):
    ...
    def _check_merge_pre(self, v):
        ...
        if ops in v:
            ...
            if isinstance(value, str):
                ...
                raise KiPlotConfigurationError(ops+" must be a string or a list of strings")
            registered = BasePreFlight.get_registered()
            for v in value:
                if v == 'all':
                    continue
                if v not in registered:
                    raise KiPlotConfigurationError(f"Unknown `{v}` preflight in `merge_pre`")
            self.merge_pre = value
    ...
    def _parse_import(self, imp, name, collected_definitions, apply=True, depth=0):
        ...
        if apply:
            ...
            BasePreFlight.add_preflights(all_collected.preflights, self.merge_pre)
    ...
    def read(self, fstream):
        ...
        self._check_merge_pre(main_sec)
        ...
        if v1:
            BasePreFlight.add_preflights(self._parse_preflights(v1), self.merge_pre)
        # Look for outputs
...
def print_output_options(name, cl, indent, context=None, skip_keys=False, skip_options=None, force_is_basic=False,
    ...
    try:
        ...
        if re.search(r'missing 2 required(.*)name(.*)value', str(e)):
            config_error(f"Wrong constructor for `{name}`, please migrate any plug-in ({e})")
        raise
    ...
    for k, v in sorted(obj.get_attrs_gen(), key=lambda x: (not obj.is_basic_option(x[0]), x[0])):
        ...
        if k != 'options':
            print(f"\n.. _{cl.__name__}_{k}:\n")
        print(preface+adapt_text(lines[0].strip()+('.' if clines == 1 and dot else ''), cl))
        if rst_mode:
            if skip_keys:
        ...
        for ln in range(1, clines):
            ...
            if rst_mode and text[-1] == '.':
                text += ' |br|'
            print(ind_help+adapt_text(text+('.' if ln+1 == clines else ''), cl))
        num_opts = num_opts+1
...
def get_doc_lines(o):
    lines = trim(adapt_to_rst_urls(o.__doc__, o))
    if len(lines) == 0:
...
def print_one_out_help(details, n, o):
    ...
    if details and GS.out_dir_in_cmd_line and rst_mode:
        ...
        with open(dest, 'wt') as f:
            f.write(RST_WARNING+'\n')
            f.write(f'.. index::\n   pair: {lines[0]}; {n}\n')
            # Extra index entries defined in the class declaration (not in __init__)
            for ps, pd in o._extra_index_pairs:
                f.write(f'   pair: {ps}; {pd}\n')
            f.write('\n')
    ...
    if rst_mode:
        ...
        if len(lines) > 1:
            ...
            if 'Important: ' in text:
                ...
                t.append('')
                text = '\n'.join(t)
        return adapt_to_rst_urls(text, cl)
    text = text.replace('\\*', '*')
    return text
...
def adapt_to_rst_urls(text, cl=None):
    if rst_mode and cl is not None:
        ...
        if SEE_REF.search(text):
            text = SEE_REF.sub(r'\1 :ref:`\2 <'+cl.__name__+r'_\2>`', text)
        # Replace "See the `OUTPUT` output" by "See :doc:`OUTPUT`"
        # This creates a reference to a separated file
        if SEE_OUT.search(text):
            text = SEE_OUT.sub(r'\1 :doc:`\2`', text)
