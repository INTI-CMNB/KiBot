...
class PanelizeFraming(PanelOptionsWithPlugin):
    def __init__(self):
        with document:
            ...
            """ [number|string=0] Length of the rail widener patch along the rail edge. See `widenercorners` """
            self.widenergap = 0
            """ [number|string=0] Minimum gap kept between the rail widener patch and the board(s). When 0 (the default)
                ...
                which would otherwise round the patch into the board and eat into this gap. See `widenercorners` """
        super().__init__()
